...
float shadow(vec3 p, vec3 lightPos, float k) {
    vec3 ro = p;
    vec3 rd = normalize(lightPos - p);
    float dist = length(lightPos - p);
    float t = marchEpsilon; // How far we've travelled along the ray
    float res = 1.;
    float brightnessInShadow = 0.7; // How bright p is when it's fully shadowed
    ...
    for (int i = 0; i < maxMarches; i++) {
        ...
        if (d < marchEpsilon) // If the shadow ray hit a surface
            return 0.;
...
vec3 shading(vec3 p, vec3 n, int steps) {
    vec3 sunLightPos = eyePos + 10.; // Spot light
    ...
    float iAmb = 0.2; // Constant ambient light
    ...
    // We could just do
    //     color *= shadow(p, sunLightPos, 14.);
    // but that would mean some parts of the scene would be completely black,
    // which would be ugly so we mix between 0.5 and 1.0, but that would make
    // the gradient of the shadow have a hard stop, so then we do a smoothstep
    // on the shadow.
    color *= mix(0.5, 1., smoothstep(0., 1., shadow(p, sunLightPos, 14.)));
    
    // Cheap ambient occlusion. Doesn't work that well. pow(..., 2) is for
    // exaggeration.
    float occlusion = pow(clamp(float(steps) * 0.0015, 0., 1.), 2.);
    color *= 1. - occlusion;
