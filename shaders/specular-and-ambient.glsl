...
const vec3 ambientLight = vec3(0.1); // Ambient ligbt color/intensity
const vec3 materialDiffuse = vec3(0.95, 0.2, 0.2);
const vec3 materialSpecular = vec3(0.7); // Material specular color/intensity
const float materialShininess = 16.0; // Material shininess
...
vec3 shade(vec3 p, vec3 n) {
    ...
    vec3 c = normalize(camPos - p); // Vector towards camera
    // Specular intensity
